...
if __name__ == '__main__':
    ...
    parser.add_argument("--infer_list", default="psa/voc12/train_aug.txt", type=str)
    ...
    for iter, (name, img) in enumerate(infer_data_loader):
        ...
        with torch.no_grad():
            if is_cuda_available:
                img = img.cuda()
            aff_mat = torch.pow(model.forward(img, True), args.beta)
            ...
            imsave(os.path.join(args.out_rw, name + '.png'), res)
